            ...
            for stat in sorted([flow for flow in body],
                ...
                try:
                    ...
                    if stat.byte_count == 0:
                        self.logger.info('%016x %34s %018.9f %018.9f %08d', ev.msg.datapath.id, rule, float("nan"),
                                         0, 0)
                ...
                self.logger.info('%016x %34s %018.9f %018.09f %08d', ev.msg.datapath.id, rule, current_rate,
                                 new_duration, new_byte_count)
